...
void SetupWizardDialog::nextPage()
{
	...
	if (current_page == Page_Complete)
	{
		if (m_ui.createDesktopShortcut->isChecked())
			QtUtils::CreateShortcut(this, "KFCSX2", std::string(), {}, std::string(), std::string(), true, false);
		if (m_ui.addToApplicationMenu->isChecked())
			QtUtils::CreateShortcut(this, "KFCSX2", std::string(), {}, std::string(), std::string(), false, false);
